...
export function computeRevenue(appointments, { from, to, source } = {}) {
  ...
  // depositOnly appointments: cash collected counts but contract value doesn't count as revenue until full payment
  const fullClosed       = closed.filter(a => !a.depositOnly)
  const contractRevenue  = fullClosed.reduce((s,a) => s + (a.contractRevenue||0), 0)
  ...
  for (const a of closed) {
    ...
    byMonthObj[m].contractRevenue  += a.depositOnly ? 0 : (a.contractRevenue||0)
    byMonthObj[m].closes++
  }
  ...
  return {
    ...
    avgDeal:         fullClosed.length ? Math.round(contractRevenue / fullClosed.length) : 0,
